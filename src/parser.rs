...
pub fn run(compile_only: bool, release: bool) {
    ...
    compiler.push_empty_line(2);
    compiler.push_line_str("// Dead code/unused variables might be generated depending on the Clade code.");
    compiler.push_line_str("#![allow(dead_code)]");
    compiler.push_line_str("#![allow(unused_variables)]");
    compiler.push_empty_line(1);
    ...
    compiler.push_empty_line(1);
    compiler.push_with_indent_str("run_clade_code(&args);");
    
    compiler.push_line_str("}");
    compiler.push_empty_line(2);

    compiler.push_line_str("fn run_clade_code(args: &Vec<(u64, Vec<(&str, &str)>)>) {");
    for code_node in &code_nodes {
        if code_node.name == "" {
            continue;
        }
